...
\end{alignat*}
onda se, iz Ojlerove formule\index{Ojlerova formula} i jednakosti \eqref{eq:lnmul} i \eqref{eq:powb}, dobija\index{ln@$\ln$}
\begin{equation}
...
\label{eq:clncart}
\end{equation}
Kako je $\e^{x+iy}=\e^x\cdot\e^{iy}$, a $\e^{iy}=\cos y+i\sin y$, sledi da je i
...
\idx{verovatno{\cc}a} da prva cifra nekog broja bude $9$ iznosi $\logten(1+1/9)\approx4\.58\%$.
