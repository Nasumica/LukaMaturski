...
\def\resenje{\par\medskip\noindent\leavevmode
  \raise\signdp\llap{$\blacktriangleright\>$}\zabox{Re{\sv}e{\nj}e:}}
\def\dodatak{\par\medskip\noindent\leavevmode
  \raise\signdp\llap{$\vcenter{\hbox{$\scriptstyle\bigstar\,$}}\>$}\zabox{Dodatak:}}
...
\subsection{Osnova 2}\index{2}\index{log2@$\logtwo$}\index{binarni logaritam}
...
Vreme {\sl poluraspada\/} $\th$\index{poluraspad} je vreme potrebno da se raspadne polovina jezgara atoma neke materije. 
...
Prvi kompjuter \idx{ENIAC} (Electronic Numerical Integrator And Computer)
...
\end{alignat*}
onda se, iz Ojlerove formule\index{Ojlerova formula} i jednakosti \eqref{eq:lnmul} i \eqref{eq:powb}, dobija
\begin{equation}
...
\subsection{Integral}
% Neodre{\dj}eni integral logaritamske funkcije je
% \begin{equation}\label{eq:integral}
%   \int \logb x\,\dx 
%   = x\left(\logb x - \frac{1}{\ln b}\right)+\const.
% \end{equation}
...
\begin{enumerate}[parsep=0cm]
  ...
  % \item \textbf{Delphi} (\pl) --- Borland, Embarcadero
...
\link{https://github.com/Nasumica/Wirth}{{\sf GitHub} --- Srbislav D. Ne{\sv}i{\cc} --- Numerical recipes in Pascal}
\end{enumerate}
