...
\iffalse
\subsection{$\textbf{\textit{i}}^\textbf{\textit{i}}$}
\zad Izra{\cv}unaj vrednost $z=i^i$.
\ans Kako $i$ ima polarne koordinate $\rho=1$ i $\theta=\pi/2$, ako ga predstavimo Ojlerovom formulom kao
$i=\e^{i\pi/2}$, sledi
$$
z=i^i=\e^{i\ln i} = \e^{i\ln \e^{i\pi/2}}=\e^{i^2\pi/2}=\ram{\e^{-\pi/2}\approx0.20788}. 
$$
\clearpage
\fi
